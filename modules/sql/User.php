...
function CreateNewUrlForUserVerify($conn): string
{
   $newUrl = RandomText(1, 21);
   $existURL = $conn->query("SELECT * FROM User WHERE urlToVerified = '$newUrl';");
   if (mysqli_fetch_array($existURL, MYSQLI_ASSOC))
      return CreateNewUrlForUserVerify($conn);
   return $newUrl;
}
function CreateNewUser($firstName, $lastName, $password, $email): array
{
   $conn = Connection::GetConnection("Synelia");
   $newUrl = CreateNewUrlForUserVerify($conn);
   if (!empty($conn)) {
